...
# 使用 playwright_helper 自动管理浏览器（优先复用 htmlrender，否则使用国内镜像下载）
from .playwright_helper import get_new_page
...
async def handle_lunar_calendar(bot: Bot, event: Event, state: T_State, args: Message = CommandArg()):
    ...
    try:
        # 解析日期
        parsed_date = DateParser.parse_strict_ymd_from_command(arg_text)
...
async def take_huangli_screenshot(bot: Bot, event: Event, date_str: str):
    ...
    try:
        ...
        async with get_new_page() as page:
            ...
            # 主动注入中文 fallback 字体，减少容器环境下网页中文缺字
            await page.add_style_tag(content=FALLBACK_FONT_CSS)
