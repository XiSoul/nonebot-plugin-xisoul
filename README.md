...
Ollama申请api网址:[https://ollama.com/settings/keys](https://ollama.com/settings/keys)
...
#### 随机图片功能
- 随机白丝图片：发送 `sjbs` 或 `/sjbs`
- 随机黑丝图片：发送 `sjhs` 或 `/sjhs`
- 随机美图：发送 `sjmt` 或 `/sjmt`
- 二次元图片：发送 `sjecy` 或 `/sjecy`
- 4K美女高青图片：发送 `sjsk` 或 `/sjsk`
...
- AI聊天：发送 `ai 问题内容`（必须是"ai+空格+问题"格式）
- 切换模型：发送 `/切换千问`、`/切换gpt` 或 `/切换deepseek`
- 查看当前模型：发送 `/当前模型`
- 查看模型列表：发送 `/模型列表`
- 清理对话历史：发送 `/清理历史` 或 `/重置对话`
- 重置模型：发送 `/重置模型`
- 获取Ollama帮助：发送 `/ollama帮助` 或 `/ollama菜单`
